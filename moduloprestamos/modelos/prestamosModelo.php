...
function buscaridd($idd){
	...
	try{
		...
		$resultado = $db->query("select * from prestamo where codLibro='".$idd."'");
		if ($db->errno != 0)
			throw new Exception('Error realizando consulta:'.$db->error, $db->errno); 
		$db->close(); 
	}catch (Exception $e){
...
function altaPrestamo($usuario1,$codLibro,$fecha){
	echo "hola";
	global $servidor, $bd, $usuario, $contrasenia;
	try{
		@ $db = new mysqli($servidor, $usuario, $contrasenia);
		...
		$consulta = "insert into prestamo (usuario,codLibro,fechaPrestamo) values ('$usuario1',$codLibro,'$fecha')";
...
/////////////////////////////////////////////////
function modificarPrestamo($usuario1,$password,$nombre,$apellido1,$apellido2,$edad,$direccion,$telefono,$dni,$email,$tipo){
	global $servidor, $bd, $usuario, $contrasenia;
	try{
		@ $db = new mysqli($servidor, $usuario, $contrasenia);
		if (mysqli_connect_errno() != 0)
